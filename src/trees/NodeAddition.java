...
public class NodeAddition {
	public static void main(String[] args) {
		...
		System.out.println("Depth First Traversal Before Addition--> ");
		depthFirstTraversal(rootNode, "-");

		Node newNode = new Node(null);
		newNode.setId("newNode");

		addNode(childNode1, newNode);

		System.out.println("Depth First Traversal After Addition--> ");
		depthFirstTraversal(rootNode, "-");
	...
	private static void addNode(Node selectedParent, Node newNode) {
		selectedParent.getChildren().add(newNode);
		newNode.setParent(selectedParent);
	}
}
